...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for row in range(cell[0] - 1, cell[0] - 2):
            for col in range(cell[0] - 1, cell[0] - 2):
                if (row, col) in self.mines:
                    mine_count += 1
                elif 0 <= row < self.height and 0 <= col < self.width and (row, col) not in self.safes and (row, col) not in self.mines:
                    undetermined.append((row, col))
        ...
        for sentence in self.knowledge:
            if new_sentence.cells.issubset(sentence.cells) and self.count > 0 and new_sentence.count > 0 and new_sentence != sentence:
                ...
                new_sent_subset = Sentence(list(new_subset), sentence.count - new_sentence.count)
                self.knowledge.append(new_sent_subset)
